...
from cleverhans_tutorials.tutorial_models import make_basic_picklable_cnn, MLP
from cleverhans.utils import AccuracyReport, set_log_level
#from cleverhans_tutorials.tutorial_models import Layer, Flatten, Linear, ReLU, Softmax
from cleverhans.picklable_model import Layer, Flatten, Linear, ReLU, Softmax, Sigmoid
...
def make_model(task):
    if task == 'mnist':
        ...
        model = make_basic_picklable_cnn(nb_classes=2)
    elif task == 'abalone':
...
class neural_net_classifier():
    ...
    def generate_adv(self, ATTACK, X_test, eps, mask=None):
        train_params = self.train_params
        nb_epochs= train_params['nb_epochs']
        batch_size=train_params['batch_size']
        learning_rate=train_params['learning_rate']
        ...
        if ATTACK == 'FGSM':
            fgsm_params = {'eps': eps,                                 #                             
                           'ord': 2}                                   # Chang the attack to deepfool       
            fgsm = FastGradientMethod(model, sess=sess)                #                    
            adv_x = fgsm.generate(x, **fgsm_params)                    #               
        elif ATTACK == 'DEEPFOOL':                                     #
            deepfool_params = {'nb_candidate': 2,                      # Set candidate to 2        
                               'overshoot': 0.02,                      #  
                               'max_iter': 50,                         # 
                               'clip_min': 0.,                         #
                               'clip_max': 1.}                         #
        ...
        adv_x = x + eps*(adv_x-x)                                      # The new adversarial example = x + scale * perturbation
        print(eps) 
        preds_adv = model.get_probs(adv_x)                      
